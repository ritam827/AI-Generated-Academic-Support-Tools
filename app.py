...
{{
    ...
    "mermaid_diagram": "A standard Mermaid flowchart for this question. Always provide one, even for definitions or calculations."
}}
...
- The mermaid_diagram field must always contain valid Mermaid.js syntax.
- Use a clear 4 to 6 node flowchart with the question, core idea, key parts, example or method, and conclusion.
- Start with 'flowchart TD'. Use quoted labels containing only letters, numbers, and spaces.
...

{
    ...
    "source_links": ["https://www.google.com/search?q=...", "https://www.google.com/search?q=..."],
    "mermaid_diagram": "A standard Mermaid flowchart for the uploaded question. Always provide one."
}
...
- Keep language simple and student-friendly.
- The source_links field must contain 2 to 5 direct Google search URLs relevant to the topic.
- The mermaid_diagram field must always be valid Mermaid.js syntax beginning with 'flowchart TD'.
- Use a clear 4 to 6 node flowchart with quoted labels containing only letters, numbers, and spaces.
- Return ONLY the JSON object, nothing else."""
...
def is_quota_error(exc):
    """Identify Gemini quota and rate-limit failures."""
...
def quota_error_response():
    ...
    return jsonify({
        'error': 'Gemini API quota is temporarily exhausted. Please wait and try again, or use a Gemini API key with available billing/quota.',
...
def upload_file():
    """Handle any uploaded file and return basic metadata."""
    try:
        ...
        print(f'Error in /upload-file: {traceback.format_exc()}')
        return jsonify({'error': str(exc).strip() or 'File upload failed.'}), 500
